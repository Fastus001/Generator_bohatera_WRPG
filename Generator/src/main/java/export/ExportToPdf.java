...
public class ExportToPdf {
    ...
    public ExportToPdf(Hero h, String des) throws IOException {
        ...
        this.hero = h;
        String nazwaPliku = hero.getName().replace( " ", "_" ) + "_" + hero.getCurrentProfessionName() + "_poz" + hero.getProfession().getLevel() + ".pdf";
    ...
    private void wczytajTalenty() {
        Iterator<Talent> talentIterator = hero.getKnownTalents().iterator();
        if ( hero.getKnownTalents().size() < 17 ) {
            for (int i = 0; i < hero.getKnownTalents().size(); i++) {
//				Talent tl = hero.knownTalents.get( i);
    ...
    private void sprawdzUmiejetnosci() {
        ...
        int ileRazyJazda = 0;
        for (Skill um : hero.getKnownSkills()) {
    ...
    private void wczytanieZywotnosci() {
        ...
        //suma ďż˝ywotnoďż˝ci
        form.getField( "fill_123" ).setValue( Integer.toString( hero.getStats().getHp() ), font1, TEN );
    ...
    private void opisBohateraCechy() {
        ...
        form.getField( "Klasa" ).setValue( profession.getCareer(), font1, 10f );

        form.getField( "Profesja" ).setValue( profession.toString(), font1, 10f );
        Integer poziom = profession.getLevel();
        zmienna = poziom.toString();

        form.getField( "Poziom Profesji" ).setValue( zmienna, font1, OSIEM );
        form.getField( "Ścieżka Profesji" ).setValue( hero.professionPathName(), font1, OSIEM );
        form.getField( "Status" ).setValue( hero.professionStatusName(), font1, OSIEM );
        ...
        for (int x = 0; x < statsCurrent.length; x++) {
            ...
            if ( statsAdvances[x] != 0 ) {
                ...
                if ( hero.getProfession().isProfessionStat( x ) ) {
