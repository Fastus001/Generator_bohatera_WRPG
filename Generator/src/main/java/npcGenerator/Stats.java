...
public class Stats implements RzutKoscia{
	...
	public Stats(int [] baseStats, RaceEnum race){
	...
	public String showStats(int [] array){
		...
		for (int i = 0; i < NO_OF_STATS; i++){
			...
			text.append( statsName ).append( ": " ).append( stats[i] ).append( " (+" ).append( advances[i] ).append( ") " );
		}
		text.append( " \nPunkty życia: " ).append( hp ).append( "\nSzybkość: " ).append( speed ).append( "\n" );
